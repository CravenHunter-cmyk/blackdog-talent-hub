...
Top navigation currently uses this order: Talent Map, BlackDog Brain, BlackDog Tools, BlackDog Talent Museum, Talent Hub, PM Hub, Sourcing Hub, Command, Account/Login. Talent Map must remain first, BlackDog Brain second, and BlackDog Tools third. Navigation names stay visible to logged-out visitors. Talent Map is public. Logged-out clicks on protected navigation items may route to the requested workspace path, but protected or unavailable workspaces should show the unified Coming Soon brand preview instead of exposing real business content, internal admin content, or modal-style access prompts. BlackDog Brain is a special protected-preview module: local development may render the full Brain workspace, while production/preview must show the Brain Coming Soon / protected preview page unless `NEXT_PUBLIC_ENABLE_BLACKDOG_BRAIN=true` enables the workspace. Command is also a protected-preview module: local development may render the full admin workspace for account, permission, delete, and batch-delete testing, while production/preview must show the unified Coming Soon brand preview unless `NEXT_PUBLIC_ENABLE_BLACKDOG_COMMAND=true` enables the real Command workspace. Production must not expose internal admin or account management by default, even to logged-in users with Command permissions. AI Diagnosis should not appear as a separate top-level navigation item; AI model evaluation diagnosis capabilities are organized under BlackDog Brain.
